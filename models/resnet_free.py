...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False, groups=1,
                 ...
                 free_model_output_dim=None, fusion=False):
        ...
        if projection:
            ...
            self.p = nn.Sequential(
                ...
                nn.BatchNorm1d(64 * block.expansion),
                nn.ReLU(inplace=True),
                nn.Linear(64 * block.expansion, 64 * block.expansion),
            )
            self.fc = nn.Linear(64 * block.expansion, num_classes)

            #not used in the output but to project the latent space features
            ...
            if self.free_model_output_dim:
                self.p_free = nn.Sequential(
                    ...
                    nn.BatchNorm1d(64 * block.expansion),
                    nn.ReLU(inplace=True),
                    nn.Linear(64 * block.expansion, free_model_output_dim),
                )
    ...
    def forward(self, x):
        ...
        if self.projection:
            ...
            #to balance the projecctor training and avoid learning deviation
        ...
        if projection:
            if self.free_model_output_dim:
                self.p_free = nn.Sequential(
                    ...
                    nn.ReLU(inplace=True),
                    nn.Linear(512 * block.expansion, free_model_output_dim),
                )
                # to balance the projecctor training and avoid learning deviation
                self.fc_free = nn.Linear(free_model_output_dim, num_classes)
...
def resnet50(class_num, pretrained=False, path=None, **kwargs):
    ...
    """
    model = ImageNet(Bottleneck, [3, 4, 6, 3], class_num, **kwargs)
    if pretrained:
